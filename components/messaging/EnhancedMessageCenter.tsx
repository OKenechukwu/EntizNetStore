...
export default function EnhancedMessageCenter({ currentUserId, userType }: MessageCenterProps) {
  const { brand, theme } = useBrand()
  const [conversations, setConversations] = useState<any[]>([])
  const [activeConversation, setActiveConversation] = useState<any>(null)
  const [loading, setLoading] = useState(true)
  const [activeCategory, setActiveCategory] = useState<MessageCategory>('inquiries')
  const [notifications, setNotifications] = useState<Record<MessageCategory, number>>({
    inquiries: 0,
    orders: 0,
    promos: 0
  })
  ...
  const categories: CategoryConfig[] = [
    {
      ...
      description: userType === 'seller' ? 'Customer questions and general inquiries' : 'Your questions and support requests'
    },
    {
      id: 'orders',
      label: 'Orders',
      ...
      description: userType === 'seller' ? 'Order-related communications with customers' : 'Your order updates and delivery notifications'
    },
    {
      id: 'promos',
      label: 'Promos',
      ...
      description: userType === 'seller' ? 'Promotional campaigns and announcements' : 'Special offers and exclusive deals'
    }
  ]
  ...
  const startConversation = async (otherUserId: string, subject: string, conversationType: string = 'general') => {
    try {
      const { data, error } = await supabase
        .from('conversations')
        .insert({
          ...
          subject: subject,
          order_id: conversationType.includes('order') ? `order_${Date.now()}` : null,
          status: 'active'
        })
        .select()
        .single()
  ...
  // Update conversations when category changes
  useEffect(() => {
    if (activeCategory) {
      setLoading(true)
    ...
    <div className="min-h-screen flex" style={{ backgroundColor: theme.colors.background }}>
      ...
      <div className="w-80 border-r flex flex-col" 
        ...
        <div className="p-4 border-b" style={{ borderColor: theme.colors.glass.border }}>
          <h1 className="text-xl font-bold mb-2" style={{ color: theme.colors.text.primary }}>
            {brand === 'primediscreet' ? 'Elite Inbox' : 'Inbox'}
          </h1>
          <p className="text-sm" style={{ color: theme.colors.text.secondary }}>
            {userType === 'seller' 
              ? (brand === 'primediscreet' ? 'Elite customer communications' : 'Customer communications')
        ...
        <div className="flex-1 overflow-y-auto">
          {activeCategoryConfig && (
            <>
              <div className="p-3 border-b" style={{ borderColor: theme.colors.glass.border }}>
        ...
        <div className="p-4 border-t" style={{ borderColor: theme.colors.glass.border }}>
          ...
          >
            {activeCategory === 'inquiries' && (brand === 'primediscreet' ? 'Elite Inquiry' : 'New Inquiry')}
