...
export default function EscrowDashboard({ sellerId }: EscrowDashboardProps) {
  ...
  const summary = useMemo(() => {
    ...
    escrowRecords.forEach((record) => {
      s.platformFees += (record.platform_fee_cents || 0) / 100;
      switch (record.status) {
        case "held": {
          const amt = (record.amount_cents || 0) / 100;
          s.totalHeld += amt;
          if (
            record.expected_release_date &&
            new Date(record.expected_release_date) <= new Date()
          ) {
            s.availableForPayout += amt;
          } else {
            s.pendingReleaseCount++;
          }
          break;
        }
        case "transferred":
        case "paid_out": {
          s.transferred += (record.amount_cents || 0) / 100;
          break;
        }
      }
    });
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        >
          ...
          <p
            ...
            style={{ color: theme.colors.accent }}
          >
            <Price amountUSD={summary.availableForPayout} />
          </p>
          <p
        ...
        >
          ...
          <p
            ...
            style={{ color: theme.colors.text.primary }}
          >
            <Price amountUSD={summary.totalHeld} />
          </p>
          <p
            className="text-sm mt-1"
            style={{ color: theme.colors.text.secondary }}
        ...
        >
          ...
          <p
            ...
            style={{ color: theme.colors.text.primary }}
          >
            <Price amountUSD={summary.transferred} />
          </p>
          <p
            className="text-sm mt-1"
            style={{ color: theme.colors.text.secondary }}
        ...
        >
          ...
          <p
            ...
            style={{ color: theme.colors.text.primary }}
          >
            <Price amountUSD={summary.platformFees} />
          </p>
          <p
            className="text-sm mt-1"
            style={{ color: theme.colors.text.secondary }}
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              ...
              >
                {brand === "primediscreet"
                  ? "Elite Payout Available"
                  : "Payout Available"}
              </h3>
              <p style={{ color: theme.colors.text.secondary }}>
                <Price amountUSD={summary.availableForPayout} /> ready for
                withdrawal
              </p>
            </div>
            <button
                    ...
                    <td className="py-3 px-4">
                      <div>
                        ...
                        <div
                          ...
                          style={{ color: theme.colors.text.secondary }}
                        >
                          {record.orders?.products?.marketplace_brand ===
                          "primediscreet"
                            ? "Prime Discreet"
                            : "EntizNet Store"}
                        </div>
                    ...
                    <td className="py-3 px-4">
                      <div>
                        <div
                          ...
                          style={{ color: theme.colors.accent }}
                        >
                          <Price amountUSD={(record.amount_cents || 0) / 100} />
                        </div>
                        <div
                          className="text-sm"
                          style={{ color: theme.colors.text.secondary }}
                        >
                          Fee:{" "}
                          <Price
                            amountUSD={(record.platform_fee_cents || 0) / 100}
                          />
                        </div>
                      </div>
                    </td>
                      ...
                      {record.status === "held" &&
                        record.expected_release_date && (
                          <div
                            ...
                            style={{ color: theme.colors.text.secondary }}
                          >
                            {new Date(record.expected_release_date) <=
                            new Date()
                              ? "Available now"
                              : `${Math.ceil(
                                  (new Date(
      ...
      >
        ...
        >
          <p>
            • <strong>Security:</strong> Funds are held safely until order
            completion
          </p>
          <p>
            • <strong>Release:</strong> Money becomes available{" "}
            {brand === "primediscreet" ? "3 days" : "7 days"} after delivery
          </p>
          <p>
            • <strong>Protection:</strong> Both buyers and sellers are protected
            from disputes
          </p>
          <p>
            • <strong>Payouts:</strong> Request withdrawals when funds are
            available
          </p>
          {brand === "primediscreet" && (
            <p>
              • <strong>Elite Benefits:</strong> Faster releases, lower fees,
              priority support
            </p>
          )}
        </div>
