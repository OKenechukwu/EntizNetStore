...
/** Basic guardrails to control spend + avoid abuse */
const MAX_CHARS_PER_REQUEST = 5000; // DeepL allows bigger, but 5k is snappy for chat
const ALLOWED_TARGETS = new Set([
  "en","de","fr","es","it","nl","pl","pt","pt-PT","pt-BR","ja","ko","zh","ru","tr","sv","cs","da","fi","hu","id","ro","sk","sl","bg","el","et","lt","lv","uk","vi","ar","hi","th" // extend if needed
]);
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (formality) body.set("formality", formality); // "default" | "more" | "less"
    if (preserveFormat) {
      // Helps keep line breaks / basic HTML intact if you pass markup
    ...
    const res = await fetch(`${HOST}/v2/translate`, {
      ...
      body,
      // NOTE: No request content is stored by DeepL on Pro plan
    });
