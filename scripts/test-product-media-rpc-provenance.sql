...
-- Structural gate: the provenance guard must be trigger-only, hardened and
-- unavailable as a browser-callable helper.
do $$
declare
...
-- Canonical promoted object for Seller A.
...
-- Cross-Seller object with valid scan evidence for Seller B.
insert into storage.objects(bucket_id, name, metadata)
values (
  'product-media',
...
-- Seller A object that exists but never passed the scanner ledger.
insert into storage.objects(bucket_id, name, metadata)
values (
  'product-media',
...
-- Seller A clean scan evidence whose promoted object has disappeared.
insert into public.upload_scan_jobs(
  id, actor_id, purpose, quarantine_path, destination_bucket, destination_path,
  declared_mime, verified_mime, byte_size, sha256, status, scanner,
  scanner_version, scanner_result_code, scanned_at, promoted_at
...
-- Seller A scan/object pair with a deliberately inconsistent extension.
insert into storage.objects(bucket_id, name, metadata)
values (
  'product-media',
...
select set_config(
  'request.jwt.claims',
  '{"sub":"a1000000-0000-4000-8000-000000000001","role":"authenticated","iss":"http://127.0.0.1:54321/auth/v1"}',
  true
);

-- Create the owned draft without media first; the subsequent calls exercise the
-- exact product_media insert trigger reached by seller_save_product_v3.
...
select pg_temp.expect_media_rejected(
  :'product_id'::uuid,
  'http://127.0.0.1:54321/storage/v1/object/public/product-media/a2000000-0000-4000-8000-000000000002/22222222-2222-4222-8222-222222222222.jpg',
  'product_media_path_not_owned'
);

select pg_temp.expect_media_rejected(
  :'product_id'::uuid,
  'http://127.0.0.1:54321/storage/v1/object/public/product-media/a1000000-0000-4000-8000-000000000001/33333333-3333-4333-8333-333333333333.png',
  'product_media_scan_provenance_required'
);

select pg_temp.expect_media_rejected(
  :'product_id'::uuid,
  'http://127.0.0.1:54321/storage/v1/object/public/product-media/a1000000-0000-4000-8000-000000000001/44444444-4444-4444-8444-444444444444.png',
  'product_media_object_missing'
);

select pg_temp.expect_media_rejected(
  :'product_id'::uuid,
  'http://127.0.0.1:54321/storage/v1/object/public/product-media/a1000000-0000-4000-8000-000000000001/55555555-5555-4555-8555-555555555555.jpg',
  'product_media_extension_mismatch'
);

select pg_temp.expect_media_rejected(
  :'product_id'::uuid,
  'http://127.0.0.1:54321/storage/v1/object/public/product-media/a1000000-0000-4000-8000-000000000001/11111111-1111-4111-8111-111111111111.webp?download=1',
  'product_media_url_not_canonical'
);

-- Authenticated calls with no trustworthy issuer fail closed.
select set_config(
  'request.jwt.claims',
...
select pg_temp.expect_media_rejected(
  :'product_id'::uuid,
  'http://127.0.0.1:54321/storage/v1/object/public/product-media/a1000000-0000-4000-8000-000000000001/11111111-1111-4111-8111-111111111111.webp',
  'product_media_auth_issuer_invalid'
);

-- Restore the canonical local issuer and prove the direct RPC accepts only the
-- real, scanned, live, owned object.
select set_config(
  'request.jwt.claims',
  '{"sub":"a1000000-0000-4000-8000-000000000001","role":"authenticated","iss":"http://127.0.0.1:54321/auth/v1"}',
  true
...
begin
  ...
  if v_url <> 'http://127.0.0.1:54321/storage/v1/object/public/product-media/a1000000-0000-4000-8000-000000000001/11111111-1111-4111-8111-111111111111.webp' then
