...
path = Path("LAUNCH_BLOCKERS.md")
source = path.read_text()


def replace_section(text: str, start: str, end: str, replacement: str) -> str:
    if text.count(start) != 1:
        raise SystemExit(f"expected exactly one start marker: {start}")
    if text.count(end) != 1:
        raise SystemExit(f"expected exactly one end marker: {end}")
...
old_m2 = (
    "Historical M2 Vercel Hobby build-rate limiting remains evidence for P0-06; "
    "later production deployments supersede the old M2 preview as current runtime evidence."
)
new_m2 = (
    "Historical M2 Vercel Hobby build-rate limiting remains recorded as an incident, "
    "but the project now runs on Vercel Pro and current production deployments "
    "supersede the old Hobby-capacity evidence."
)
if source.count(old_m2) != 1:
    raise SystemExit("expected historical Vercel Hobby sentence once")
source = source.replace(old_m2, new_m2, 1)

p004 = """## P0-04 — Authorization/RLS HTTP regression coverage
...
p005 = """## P0-05 — Seller/Admin/KYC/storage security completion
...
p006 = """## P0-06 — Production deployment and migration hardening
...
p007 = """## P0-07 — Observability and commerce incident response
...
p010 = """## P0-10 — Responsive web and accessibility public-launch pass
...
if source.count(marker) != 1:
    raise SystemExit("expected P1 boundary exactly once")
...
native = """## P1-01 — Native iOS/Android marketplace client
